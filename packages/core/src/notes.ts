...
export type SelectionResult =
  | { ok: true; notes: SelectableNote[]; total: bigint; change: bigint; exact: boolean }
  | {
      ok: false;
 ...
 * never the balance.
 */
export function planLadder(balance: bigint, options: LadderOptions): LadderPlan {
 ...
 */
export function ladderGaps(held: readonly SelectableNote[], options: LadderOptions): bigint[] {
  ...
  for (const denom of options.denoms) {
    ...
    for (let i = 0; i < missing; i++) gaps.push(denom);
  }
  return gaps;
}
...
export function selectNotes(
  ...
  for (const note of notes) {
    ...
    if (isMature) mature.push(note);
    else immatureTotal += note.amount;
  }
...
function findExactSubset(
  ...
  const suffixTotals: bigint[] = new Array(sorted.length + 1).fill(0n);
  for (let i = sorted.length - 1; i >= 0; i--) {
    suffixTotals[i] = suffixTotals[i + 1]! + sorted[i]!.amount;
  }

  let steps = 0;
  const chosen: SelectableNote[] = [];

  function search(index: number, remaining: bigint): boolean {
    if (remaining === 0n) return true;
    if (index >= sorted.length || remaining < 0n) return false;
    if (suffixTotals[index]! < remaining) return false;
    if (++steps > budget) return false;

    const note = sorted[index]!;
    if (note.amount <= remaining) {
      chosen.push(note);
      if (search(index + 1, remaining - note.amount)) return true;
      chosen.pop();
    }
