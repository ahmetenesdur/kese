 ...
 */
export function resolveSigner(env = process.env) {
    const missing = [];
    const address = read(env, "ACCOUNT_ADDRESS");
    if (!address)
        missing.push("ACCOUNT_ADDRESS");
    const privateKey = read(env, "ACCOUNT_PRIVATE_KEY");
    if (!privateKey)
        missing.push("ACCOUNT_PRIVATE_KEY");
    const viewingKeyRaw = read(env, "VIEWING_KEY");
    if (!viewingKeyRaw)
        missing.push("VIEWING_KEY");
    if (!address || !privateKey || !viewingKeyRaw)
        return { value: null, missing };
    let viewingKey;
    try {
